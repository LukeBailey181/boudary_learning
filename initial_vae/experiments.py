...
MNIST_TRAIN_PATH = "../data/binary_MNIST/bin_mnist_train.pkl"
MNIST_TEST_PATH = "../data/binary_MNIST/bin_mnist_test.pkl"

MNIST_ELBO_DATA_PATH = "../data/ordered_data/eblo.pkl"
MNIST_CLASS_ELBO_DATA_PATH = "../data/ordered_data/class_eblo.pkl"
MNIST_NEIGHBORS_DATA_PATH = "../data/ordered_data/latent_neighbors.pkl"
MNIST_GAUSSIAN_DATA_PATH = "../data/ordered_data/gaussian_data.pkl"
MNIST_ENTROPY_DATA_PATH = "../data/ordered_data/entropy_data.pkl"
MNIST_TOP_PROB_DIFF_DATA_PATH = "../data/ordered_data/top_prob_diff_data.pkl"
MNIST_P_P_COMP_DATA_PATH = "../data/ordered_data/p_p_comp_data.plk"

CIFAR10_ELBO_DATA_PATH = "../data/ordered_data/cifar10_eblo.pkl"
CIFAR10_CLASS_ELBO_DATA_PATH = "../data/ordered_data/cifar10_class_eblo.pkl"
CIFAR10_NEIGHBORS_DATA_PATH = "../data/ordered_data/cifar10_latent_neighbors.pkl"
CIFAR10_GAUSSIAN_DATA_PATH = "../data/ordered_data/cifar10_gaussian_data.pkl"
CIFAR10_ENTROPY_DATA_PATH = "../data/ordered_data/cifar10_entropy_data.pkl"
CIFAR10_TOP_PROB_DIFF_DATA_PATH = "../data/ordered_data/cifar10_top_prob_diff_data.pkl"
CIFAR10_P_P_COMP_DATA_PATH = "../data/ordered_data/cifar10_p_p_comp_data.plk"

MNIST_VAE_MODEL_PATH = "./models/mnist_vae.pkl"
...
def train_and_test_on_dataset(
    ...
    epoch_losses = train_net(
        epochs, net, train_loader, preproc=True, lr=lr, dataset_name=dataset_name
    )
...
def test_boundary_learning(
    ...
    if dataset_name == "mnist":
        if classes is None:
            ...
            trainset, testset = load_class_spec_mnist(
                TRAIN_PATH, TEST_PATH, classes=classes
            )
        ...
        if classes is None:
            ...
            raise NotImplementedError(
                "Not yet implemented class specific cifar10 testing"
            )

        # raise NotImplementedError("Not yet implemented CIFAR VAE")
    ...
    if classes is None:
        ...
        entropy_data, _ = sort_dataset_by_entropy(
            trainset, testset, load_path=ENTROPY_DATA_PATH, dataset_name=dataset_name
        )
        p_p_comp_data, _ = sort_dataset_by_p_p_comp(
            trainset, testset, load_path=P_P_COMP_DATA_PATH, dataset_name=dataset_name
        )
        top_prob_diff_data, _ = sort_dataset_by_top_prob_diff(
            trainset,
            testset,
            load_path=TOP_PROB_DIFF_DATA_PATH,
        ...
        entropy_data, _ = sort_dataset_by_entropy(
            trainset, testset, num_classes=num_classes
        )
        p_p_comp_data, _ = sort_dataset_by_p_p_comp(
            trainset, testset, num_classes=num_classes
        )
    ...
    for prop in props:
        ...
        train_data_dict["entropy_prune"] = flatten_dataset(
            ordered_prune(entropy_data, prop)
        )
        # train_data_dict["entropy_prune_50_50"] = flatten_dataset(ordered_prune(entropy_data, prop, prop_random=0.5))
        train_data_dict["p_p_comp_prune"] = flatten_dataset(
            ordered_prune(p_p_comp_data, prop)
        )
        train_data_dict["top_prob_diff_prune"] = flatten_dataset(
            ordered_prune(top_prob_diff_data, prop)
        )
...
def visualize_pruned_data(k=1000, prop=0.01):
    ...
    # Collect sorted data
    trainset, testset = load_binary_mnist(1, TRAIN_PATH, TEST_PATH)
    class_elbo_data, _ = sort_dataset_by_class_elbo(
        vae_dict, trainset, k=k, load_path=CLASS_ELBO_DATA_PATH
    )
    elbo_data, _ = sort_dataset_by_elbo(
        multi_class_vae, trainset, k=k, load_path=ELBO_DATA_PATH
    )
    nn_data, _ = sort_dataset_by_latent_neighbors(
        vae_dict, trainset, load_path=NEIGHBORS_DATA_PATH
    )
    gaussian_data, _ = sort_dataset_by_fitted_gaussian(
        vae_dict, trainset, load_path=GAUSSIAN_DATA_PATH
    )
    entropy_data, _ = sort_dataset_by_entropy(
        trainset, testset, load_path=ENTROPY_DATA_PATH
    )
...
def test_metric_against_elbo(
    ...
    if use_dropout:
        net = make_dropoout_net(
            num_classes=10, input_dim=784, hidden_units=1200, hidden_layers=2, p=p
        )
    else:
        net = make_standard_net(
            num_classes=10,
    ...
    if use_testset:
        ...
        class_elbo_data, class_elbos = sort_dataset_by_class_elbo(
            vae_dict, trainset, k, load_path=CLASS_ELBO_DATA_PATH
        )
...
def test_entropy_against_pruning_technique(
    ...
    elbo_data, elbos = sort_dataset_by_elbo(
        multi_class_vae, trainset, k, load_path=ELBO_DATA_PATH
    )
    nn_data, distances = sort_dataset_by_latent_neighbors(
        vae_dict, trainset, load_path=NEIGHBORS_DATA_PATH
    )
    gaussian_data, likelihood = sort_dataset_by_fitted_gaussian(
        vae_dict, trainset, load_path=GAUSSIAN_DATA_PATH
    )
...
def sort_datasets(k=1000):
    ...
    trainset, testset = get_cifar10()
    sort_dataset_by_entropy(trainset, testset, save_path=CIFAR10_ENTROPY_DATA_PATH, dataset_name="cifar10")
    #sort_dataset_by_p_p_comp(trainset, testset, save_path=CIFAR10_P_P_COMP_DATA_PATH, dataset_name="cifar10")
    #sort_dataset_by_top_prob_diff(trainset, testset, save_path=CIFAR10_TOP_PROB_DIFF_DATA_PATH, dataset_name="cifar10")
